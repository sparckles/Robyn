                    ...
                    match gen.call_method0("__next__") {
                        Ok(value) => {
                            match value.extract::<String>() {
                                ...
                                Err(extract_err) => {
                                    log::debug!("Failed to extract string from generator value: {}", extract_err);
                                    None // End of stream
                                }
                            }
                        }
    ...
    pub fn new(
        ...
        let headers_output: Py<Headers> = if let Some(headers) = headers {
            ...
            if media_type == "text/event-stream" {
                ...
                headers.set("Access-Control-Allow-Headers".to_string(), "Cache-Control".to_string());
            } else {
...
impl FromPyObject<'_> for Response {
    fn extract_bound(obj: &Bound<'_, PyAny>) -> PyResult<Self> {
        ...
        if type_name != "Response" {
            ...
            return Err(PyErr::new::<pyo3::exceptions::PyTypeError, _>(
                format!("Expected Response, got {}", type_name)
            ));
        }
        ...
        log::debug!("Successfully extracted Response with status {}", status_code);
        Ok(Response {
            status_code,
            response_type,
            headers,
...
impl FromPyObject<'_> for StreamingResponse {
    fn extract_bound(obj: &Bound<'_, PyAny>) -> PyResult<Self> {
        ...
        log::debug!("Attempting to extract StreamingResponse from type: {}", type_name);
        ...
        log::debug!("Attribute check: content={}, status_code={}, headers={}, media_type={}", 
        ...
        if !has_content || !has_status_code || !has_headers || !has_media_type {
            ...
            return Err(PyErr::new::<pyo3::exceptions::PyTypeError, _>(
                format!("Object is missing required StreamingResponse attributes")
            ));
        }
        ...
        if media_type == "text/event-stream" {
            ...
            if headers.get("Access-Control-Allow-Origin".to_string()).is_none() {
                headers.set("Access-Control-Allow-Origin".to_string(), "*".to_string());
            }
            if headers.get("Access-Control-Allow-Headers".to_string()).is_none() {
                headers.set("Access-Control-Allow-Headers".to_string(), "Cache-Control".to_string());
            }
        }
        ...
        log::debug!("Successfully extracted StreamingResponse with status {} from type {}", status_code, type_name);
