...
<Row>
  ...
  <Col sticky>
    ...
    @app.post("/jsonify/:id")
    async def json(path_params: PathParams):
        print(path_params["id"])
        return {"hello": "world"}
...
<Row>
  ...
  <Col sticky>
    ...
    ```python {{ title: 'untyped' }}
    @app.get("/query")
    async def query_get(query_params):
    ...
    @app.get("/query")
    async def query_get(query_params: QueryParams):
        query_data = query_params.to_dict()
        return jsonify(query_data)
    ```
...
<Row>
  ...
  Any request param can be used in the handler function either using type annotations or using the reserved names. Please note that the type annotations will take precedence over the reserved names.
