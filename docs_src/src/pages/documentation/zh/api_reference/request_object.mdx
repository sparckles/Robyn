...
查询参数的名称和值会按 UTF-8 进行一次百分号解码。在查询字符串中，`+` 会转换为空格，`%2B` 会转换为字面量 `+`。例如，`?first%20name=Ada+Lovelace` 对应的 `request.query_params.to_dict()` 为 `{"first name": ["Ada Lovelace"]}`。

重复查询参数的所有值都会按顺序保留。`get()` 和 `items()` 返回每个解码后名称的最后一个值；`get_all()` 和 `to_dict()` 保留所有值。

路径参数值会在路由匹配之后进行百分号解码，路由中间件也遵循相同行为。路径中的字面量 `+` 保持不变。对于路由 `/items/:id`，请求 `/items/a%2Fb` 会得到 `request.path_params["id"] == "a/b"`，而 `request.url.path` 仍为 `/items/a%2Fb`。因此，编码后的斜杠不会改变已经匹配的路径分段。解码仅执行一次：`%252F` 会变为 `%2F`。

由于解码在路由匹配之后进行，单个 `:param` 捕获在解码后可能包含 `/` 或 `..`，这与 `*catch-all` 相同。在用路径参数拼接文件系统路径之前，请先进行校验。

无效的 UTF-8 字节序列会被替换为 `U+FFFD`。无效或不完整的百分号转义会保留原样。WebSocket 的 `query_params` 遵循相同规则。
...
Robyn 提供了一组便捷的类型别名，用于描述请求各组成部分的运行时类型，这样你就可以为处理函数添加类型注解，并通过 `mypy`/`pyright` 进行类型检查。它们既可以从 `robyn` 导入，也可以从 `robyn.types` 导入。

<Row>
<Col>
<ul>
<li>
`RequestMethod`：`Literal["GET", "POST", "PUT", "DELETE", "PATCH", "HEAD", "OPTIONS", "CONNECT", "TRACE"]` —— 即 `request.method` 的取值。
</li>
<li>
`RequestBody`：`str | bytes` —— 即 `request.body` 的类型（UTF-8 文本为 `str`，否则为 `bytes`）。
</li>
<li>
`RequestURL`：`Url` 的别名 —— 即 `request.url` 的类型。
</li>
</ul>
</Col>
<Col sticky>

<CodeGroup title="类型别名" tag="GET" label="/typed">
