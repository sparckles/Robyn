...
如果您已安装 Pydantic（`pip install "robyn[pydantic]"`），可以直接使用 Pydantic `BaseModel` 类作为处理函数参数的类型注解。Robyn 将自动验证请求体，**并且**生成丰富的 OpenAPI Schema — 包括属性类型、必填字段、默认值，以及嵌套模型的 `$ref` 引用。
