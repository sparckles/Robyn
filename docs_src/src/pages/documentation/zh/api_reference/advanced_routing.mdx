...
<Row>
  <Col>
    ...
    子路由器只需要一个 `prefix`（也可以按位置传入，例如 `SubRouter("/api/v1")`）。你还可以传入 `tags`，为该子路由器在 OpenAPI 文档中贡献的每个路由打标签。嵌套的子路由器会累积其父级的前缀和标签。

    <Note>
      早期版本要求将模块名作为第一个参数（`SubRouter(__name__, prefix="/api/v1")`）。该参数已不再需要；如果传入，将被弃用并忽略。
    </Note>
  </Col>
  <Col sticky>
...
- [性能优化](/documentation/zh/api_reference/performance_optimization)
