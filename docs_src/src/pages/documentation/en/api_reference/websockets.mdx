...
<Row>
  <Col>
    ...
    | `await websocket.receive_json()` | Same but JSON-decoded |
...
## Legacy API {{ tag: 'Legacy', label: 'Legacy' }}

<Row>
  <Col>
    The old event-based WebSocket API is still supported for backward compatibility. If you have existing code using `WebSocket(app, "/ws")` with `@websocket.on("message")`, it will continue to work without changes.
  </Col>
  <Col sticky>
    <CodeGroup title="Legacy API" tag="WebSocket" label="/web_socket">

      ```python {{ title: 'Legacy Style' }}
