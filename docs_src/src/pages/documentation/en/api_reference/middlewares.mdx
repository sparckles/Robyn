...
<Row>
  ...
  The `before_request` chain runs top to bottom and stops early if any handler returns a `Response` (the route handler and the remaining middlewares are then skipped). The `after_request` chain runs over the response, also in registration order.
  ...
  <Col sticky>
    ...
    <CodeGroup title="Multiple middlewares" tag="GET" label="/index">
