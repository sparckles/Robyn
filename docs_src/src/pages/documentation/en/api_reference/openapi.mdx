...
Every route decorator (`@app.get`, `@app.post`, … and the `SubRouter` equivalents) accepts a set of FastAPI-style flags so you can document — and in some cases change the runtime behaviour of — a route directly from the decorator:
