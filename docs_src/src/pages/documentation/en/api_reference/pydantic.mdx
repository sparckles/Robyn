...
Pydantic validation works with every HTTP method that accepts a request body — `POST`, `PUT`, `PATCH`, and `DELETE`. The annotation-based approach is the same for all methods:

<CodeGroup title="HTTP Methods">
