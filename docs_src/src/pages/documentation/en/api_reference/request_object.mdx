...
## Query and Path Parameter Decoding

Query parameter names and values are percent-decoded once using UTF-8. In query strings, `+` becomes a space and `%2B` becomes a literal `+`. For example, `?first%20name=Ada+Lovelace` produces `{"first name": ["Ada Lovelace"]}` from `request.query_params.to_dict()`.

Repeated query parameters keep all their values in order. `get()` and `items()` return the last value for each decoded key; `get_all()` and `to_dict()` preserve every value.

Path parameter values are percent-decoded after route matching, including in route middleware. A literal `+` in a path stays `+`. For a route `/items/:id`, `/items/a%2Fb` produces `request.path_params["id"] == "a/b"` while `request.url.path` remains `/items/a%2Fb`. An encoded slash therefore stays within its matched parameter. Decoding happens once: `%252F` becomes `%2F`.

Because decoding happens after matching, a single `:param` capture can contain `/` or `..` once decoded, just like a `*catch-all`. Sanitize path parameters before using them to build filesystem paths.

Invalid UTF-8 byte sequences are replaced with `U+FFFD`. Invalid or incomplete percent escapes remain literal. WebSocket `query_params` follow the same rules.
...
<CodeGroup title="Type Aliases" tag="GET" label="/typed">
