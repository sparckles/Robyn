...
def setup_app():
    ...
    @app.get("/query/typed")
    def query_typed(name: str = Query(...), age: int = Query(...), active: bool = Query(True), score: Optional[float] = Query(None)):
    ...
    def create_user(user: User) -> dict:
        return {"received": user.dict(), "name_upper": user.name.upper(), "is_adult": user.age >= 18}

    @app.post("/pydantic/nested")
    def nested_user(user: User, user_id: int = Query(...)) -> dict:
        return {"user": user.dict(), "user_id": user_id}

    # Test mixed parameters
    @app.get("/mixed/:item_id")
    def mixed_params(item_id: int = Path(...), name: str = Query(...), x_token: Optional[str] = Header(None), limit: int = Query(10)):
        return {"item_id": item_id, "name": name, "token": x_token, "limit": limit}
    ...
    def legacy_split(path_params, query_params):
        return {"id": path_params.get("id"), "params": query_params.to_dict()}
...
def test_query_typed():
    ...
    def test_handler(name: str = Query(...), age: int = Query(...), active: bool = Query(True)):
...
def test_mixed_params():
    ...
    def test_handler(item_id: int = Path(...), name: str = Query(...), x_token: Optional[str] = Header(None), limit: int = Query(10)):
        return {"item_id": item_id, "name": name, "token": x_token, "limit": limit}
