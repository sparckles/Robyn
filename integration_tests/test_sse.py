...
def test_sse_basic_stream(session):
    ...
    with requests.get(f"{BASE_URL}/sse/basic", stream=True, timeout=5) as response:
...
def test_sse_formatted_messages(session):
    ...
    with requests.get(f"{BASE_URL}/sse/formatted", stream=True, timeout=5) as response:
        response.raise_for_status()
...
def test_sse_json_data(session):
    ...
    with requests.get(f"{BASE_URL}/sse/json", stream=True, timeout=5) as response:
        response.raise_for_status()
...
def test_sse_named_events(session):
    ...
    with requests.get(f"{BASE_URL}/sse/named_events", stream=True, timeout=5) as response:
        response.raise_for_status()
...
def test_sse_async_endpoint(session):
    ...
    with requests.get(f"{BASE_URL}/sse/async", stream=True, timeout=5) as response:
        response.raise_for_status()
...
def test_sse_single_message(session):
    ...
    with requests.get(f"{BASE_URL}/sse/single", stream=True, timeout=3) as response:
        response.raise_for_status()
...
def test_sse_empty_stream(session):
    ...
    with requests.get(f"{BASE_URL}/sse/empty", stream=True, timeout=2) as response:
        response.raise_for_status()
...
def test_sse_streaming_sync_real_time(session):
    ...
    with requests.get(f"{BASE_URL}/sse/streaming_sync", stream=True, timeout=10) as response:
        response.raise_for_status()
...
def test_sse_streaming_async_real_time(session):
    ...
    start_time = time.time()
    with requests.get(f"{BASE_URL}/sse/streaming_async", stream=True, timeout=10) as response:
        response.raise_for_status()
