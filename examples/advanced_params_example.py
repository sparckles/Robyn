...
def path_example(user_id: int = Path(..., description="User ID"), score: float = Path(..., description="User score")):
    """Example with typed path parameters."""
    return {"user_id": user_id, "score": score, "grade": "A" if score >= 90 else "B" if score >= 80 else "C"}
...
@app.get("/header")
def header_example(x_token: Optional[str] = Header(None, description="API token"), user_agent: Optional[str] = Header(None, description="User agent")):
    """Example with header parameters."""
    return {"authenticated": x_token is not None, "token": x_token, "user_agent": user_agent}


@app.get("/header/custom")
def header_custom_example(api_key: str = Header(..., alias="X-API-Key"), request_id: Optional[str] = Header(None, alias="X-Request-ID")):
    """Example with custom header names."""
    return {"api_key": api_key, "request_id": request_id}
...
def create_user(user: User) -> dict:
    ...
    return {
        "received": user.dict(),
        "processed": {"name_upper": user.name.upper(), "is_adult": user.age >= 18, "tag_count": len(user.tags) if user.tags else 0},
    }
...
def mixed_with_body(user: User, user_id: int = Path(...), notify: bool = Query(True), x_source: Optional[str] = Header(None, alias="X-Source")):
    """Example with path, query, header, and body parameters."""
    return {"user_id": user_id, "user": user.dict(), "notify": notify, "source": x_source, "action": f"Updated user {user_id} with data for {user.name}"}
...
def legacy_request(request):
    ...
    return {"method": request.method, "path": request.url.path, "headers": dict(request.headers)}
...
def index():
    ...
    return {
        ...
        "examples": {"user_json": {"name": "John Doe", "email": "john@example.com", "age": 30, "active": True, "tags": ["python", "web", "api"]}},
    }
